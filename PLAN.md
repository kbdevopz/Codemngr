...
| **D**  | First-run import scanner      | 3     | 3–4 hr | —                   | ✅ Done. New `server.detectExistingProviderHomes` RPC scans `~/.claude` and `~/.codex` server-side, reports `{exists, alreadyConfigured}` per candidate. "Detect existing accounts" button in settings opens a modal with checkboxes; importing adds a `providerInstances.<driver>_imported[_n]` entry whose `homePath` points at the existing directory — no copy, no symlink. Auto-deduplicates against existing instances configured for the same path. Detection is shallow (existence-only); the auth probe stays where it lived in T3. |
| **E**  | Shared settings symlink layer | 4     | 3–4 hr | A or B              | ⚠ Partial — `settings.json` only. New `sharedSettingsManager` server module + 9 unit tests, three RPCs (`get/enable/disable`), and a "Shared settings.json" panel on Claude/Codex instance cards. First instance to enable seeds shared content from local; subsequent instances link to the shared file (with `.backup-<ts>` of any divergent local). Disable copies shared content back to a real local file. Directories (`hooks/`, `agents/`) deferred — they need per-file conflict UX.                                                  |
